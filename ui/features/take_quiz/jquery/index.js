    ...
    return {
      ...
      showWarnings(currentTimeLeft) {
        if (
          ...
          currentTimeLeft < 60000 &&
          !quizSubmission.oneMinuteDeadline
        ) {
          quizSubmission.oneMinuteDeadline = true
          $.flashWarning(
            I18n.t(
              'notices.submission_one_minute_left',
              'This Quiz Will Be Submitted In One Minute',
            ),
            5000,
          )
        } else if (
      ...
      showTimeUpDialog(now) {
        ...
        $.screenReaderFlashMessage(
          I18n.t('times_up', "Time's up! Submitting results in 10 seconds"),
        )
        $('#times_up_dialog')
          .show()
          .dialog({
      ...
      updateQuestionIndicators(answer, questionId) {
        ...
        if (answer) {
          ...
          $questionIcon
            .find('.icon-text')
            .text(I18n.t('question_unanswered', "Haven't Answered Yet"))
        }
      },

      updateNextButtonState(id) {
        const $question = $('#' + id)
        quizSubmission.toggleActiveButtonState(
          'button.next-question',
          $question.hasClass('answered'),
        )
      },
